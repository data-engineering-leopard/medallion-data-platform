...
def single_customer_df(spark, customer_schema):
    ...
    return spark.createDataFrame(
        [(1, "ALICE", "alice@email.com", "active", "UK")], customer_schema
    )
...
def existing_single_customer(spark, customer_schema, empty_customer_df):
    ...
    incoming = spark.createDataFrame(
        [(1, "ALICE", "alice@old.com", "active", "UK")], customer_schema
    )
    return apply_scd2(
        spark=spark,
...
def changed_record_result(spark, customer_schema, existing_single_customer):
    ...
    incoming = spark.createDataFrame(
        [(1, "ALICE", "alice@new.com", "active", "UK")], customer_schema
    )
    return apply_scd2(
        spark=spark,
        incoming_df=incoming,
...
class TestSeparateQuarantineRecords:
    ...
    def test_mixed_records_split_correctly(self, spark, customer_schema_with_dates):
    ...
    def test_returns_all_records_when_no_date_columns(self, spark, customer_schema):
        ...
        result = separate_quarantine_records(
            df, effective_from_column=None, effective_from_fallback_column=None
        )
...
class TestResolveEffectiveFrom:
    ...
    def test_uses_updated_date_when_present(self, spark, customer_schema_with_dates):
    ...
    def test_uses_hardcoded_ts_when_no_date_columns(self, spark, customer_schema):
    ...
    def test_adds_effective_from_column(self, spark, customer_schema_with_dates):
    ...
    def test_adds_effective_to_column(self, spark, customer_schema_with_dates):
    ...
    def test_adds_is_current_column(self, spark, customer_schema_with_dates):
...
class TestScd2NewRecords:
    ...
    def test_new_records_have_effective_to_as_max_date(self, new_record_result):
...
class TestScd2ChangedRecords:
    ...
    def test_changed_record_creates_new_version(self, changed_record_result):
...
class TestScd2UnchangedRecords:
    ...
    def test_unchanged_record_not_duplicated(self, unchanged_record_result):
        """A record with no changes should not be duplicated"""
        assert unchanged_record_result.count() == 1

    def test_unchanged_record_remains_current(self, unchanged_record_result):
...
class TestScd2MultipleRecords:
    ...
    def test_multiple_customers_handled_independently(self, multiple_record_result):
...
class TestScd2EventDate:
    ...
    def updated_date_result(
        self, spark, customer_schema_with_dates, empty_customer_with_dates_df
    ):
    ...
    def test_uses_updated_date_as_effective_from(self, updated_date_result):
...
class TestScd2Quarantine:
    ...
    def test_records_missing_both_dates_are_quarantined(self, mixed_quarantine_result):
    ...
    def test_quarantined_records_not_in_silver(self, all_quarantine_result):
        """Quarantined records should not appear in valid output"""
        assert all_quarantine_result["valid"].count() == 0

    def test_quarantined_records_captured_separately(self, mixed_quarantine_result):
