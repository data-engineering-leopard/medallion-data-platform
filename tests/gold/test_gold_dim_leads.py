...
from my_project.tasks.gold.dim_leads import build_dim_leads, run_dim_leads
...
class TestBuildDimLeads:
    ...
    def test_all_silver_columns_present(self, spark, silver_leads_schema):
        ...
        assert "updated_date" in result.columns
...
class TestRunDimLeads:
    ...
    def test_run_dim_leads_creates_output(self, spark, tmp_path, silver_leads_schema):
        ...
        spark.createDataFrame(data, silver_leads_schema).write.parquet(silver_path)

        run_dim_leads(spark, input_path=silver_path, output_path=gold_path)

        assert os.path.exists(gold_path)

    def test_run_dim_leads_output_readable(self, spark, tmp_path, silver_leads_schema):
